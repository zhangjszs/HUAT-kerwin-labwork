 ...
 * @brief 创建链表头节点
 *
 * @return struct Node* 头节点指针，失败时程序退出
 */
struct Node* createHead(void) {
...
void saveInfoFile(const char* fileName, struct Node* headNode) {
    ...
    while (pMove != NULL) {
        fprintf(fp, "%s\t%s\t%s\t%d\t%s\t%s\n", pMove->data.ServiceStationName, pMove->data.id,
                pMove->data.ClaimTime, pMove->data.ClaimAmount, pMove->data.CarVIN,
                pMove->data.Reviewer);
        pMove = pMove->next;
    }
