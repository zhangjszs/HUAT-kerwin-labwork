...
def main():
    ...
    plt.subplots_adjust(left=0.1, bottom=0.30, hspace=0.3)
    ...
    line_red, = ax1.plot(queue_lengths, red_probs, 'b-', lw=2, label='RED')
    line_ared, = ax1.plot(queue_lengths, ared_probs, 'r--', lw=2, label='ARED (current max_p)')
    ax1.set_xlabel('Queue Length')
    ax1.set_ylabel('Drop Probability')
    ax1.set_title('RED vs ARED Drop Probability Curves')
    ax1.legend()
    ax1.grid(True)
    ...
    def simulate_ared_trajectory(ared_sim):
        """模拟正弦队列长度变化下的 max_p 调整轨迹"""
        sim = AREDSimulator(ared_sim.alpha, ared_sim.beta, ared_sim.max_p,
                            ared_sim.min_th, ared_sim.max_th)
        max_p_values = []
        queue_trace = []
        for t in time_points:
            q = sim.min_th + (sim.max_th - sim.min_th) * (0.5 + 0.6 * np.sin(t / 5))
            sim.update_max_p(q)
            max_p_values.append(sim.max_p)
            queue_trace.append(q)
        return max_p_values, queue_trace

    max_p_vals, q_trace = simulate_ared_trajectory(ared)
    line_maxp, = ax2.plot(time_points, max_p_vals, 'g-', lw=2, label='max_p')
    ax2_twin = ax2.twinx()
    line_queue, = ax2_twin.plot(time_points, q_trace, 'c--', lw=1, alpha=0.5, label='Queue Length')
    ax2.set_xlabel('Time')
    ax2.set_ylabel('max_p', color='g')
    ax2_twin.set_ylabel('Queue Length', color='c')
    ...
    ax_max_p = plt.axes([0.1, 0.20, 0.65, 0.03], facecolor=axcolor)
    ax_min_th = plt.axes([0.1, 0.15, 0.65, 0.03], facecolor=axcolor)
    ax_max_th = plt.axes([0.1, 0.10, 0.65, 0.03], facecolor=axcolor)
    ax_alpha = plt.axes([0.1, 0.05, 0.65, 0.03], facecolor=axcolor)
    ax_beta = plt.axes([0.1, 0.00, 0.65, 0.03], facecolor=axcolor)
